...
public class Complex {
    ...
    public void quickSort(Object arr[], int low, int high) {
        int i = low;
        int j = high;
        Object pivot = arr[(low + high) / 2];
        do {
            //while(arr[i]<pivot) i++;
            while (util.Utility.lessT(arr[i], pivot)) {
                i++;
            }
            //while(arr[j]>pivot) j--;
            while (util.Utility.greaterT(arr[j], pivot)) {
                j--;
            }
            if (i <= j) {
                Object aux = arr[i];
    ...
    public void mergeSort(Object a[], Object tmp[], int low, int high) {
        if (low < high) {
            int center = (low + high) / 2;
            mergeSort(a, tmp, low, center);
            mergeSort(a, tmp, center + 1, high);
    ...
    private void merge(Object a[], Object tmp[], int lowIndex, int highIndex, int endIndex) {
        int leftEnd = highIndex - 1;
        int tmpPos = lowIndex;
        int numElements = endIndex - lowIndex + 1;
        while (lowIndex <= leftEnd && highIndex <= endIndex) //if(a[lowIndex]<=a[highIndex])
        {
            if (util.Utility.lessOrEqualT(a[lowIndex], a[highIndex])) {
                tmp[tmpPos++] = a[lowIndex++];
            } else {
                tmp[tmpPos++] = a[highIndex++];
            }
    ...
    public void shellSort(Object a[]) {
        ...
        for (int gap = n / 2; gap > 0; gap /= 2) {
            ...
            for (int i = gap; i < n; i += 1) {
                ...
                int j;
                //for (j = i; j >= gap && a[j - gap] > temp; j -= gap) 
                for (j = i; j >= gap && util.Utility.greaterT(a[j - gap], temp); j -= gap) {
                    a[j] = a[j - gap];
                }
