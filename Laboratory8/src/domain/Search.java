...
public class Search {
	...
	//if(element>a[a.length-1])
        if(util.Utility.greaterT(element, a[a.length-1]))
		return -1;
        for(int i=0;i<a.length;i++){
           //if(a[i]==element)
           if(util.Utility.equals(a[i], element))
                return i;
        }
        return -1;
    }
    ...
    if(low>high)
       ...
       //if(value==sortedArray[middle])
        if(util.Utility.equals(value, sortedArray[middle]))
          return middle;
       else if(util.Utility.lessT(value, sortedArray[middle]))
              return binarySearch(sortedArray, value, low, middle-1);
          else
              return binarySearch (sortedArray, value, middle+1, high);
    }
    ...
    public int binarySearch(Object sortedArray[], Object value){
 	...
 	int high = sortedArray.length-1;
 	int middle = (low+high)/2;
 	while ((low<=high) && !util.Utility.equals(sortedArray[middle], value)){
            if(util.Utility.lessT(value, sortedArray[middle])) 
                 high = middle - 1;
             else
 	       low = middle + 1;
 	    middle = (low+high)/2;
